...
UWORD crc_table16[256];

struct IOExtTD iotd;
struct TDU tdu0;
struct TDU tdu1;
struct timerequest timerIO2;
struct MsgPort timerMP2;
struct TrackDiskBase tdb0;
...
WORD amiga_floprw(LONG buf, WORD rw, WORD dev, WORD sect, WORD track, WORD side, WORD count)
{
    ...
    D(bug("amiga_floprw buf=0x%08lx track=%d side=%d sect=%d count=%d offset=%lu\n", buf, track, side, sect, count, iotd.iotd_Req.io_Offset));
    if (rw & 1) {
