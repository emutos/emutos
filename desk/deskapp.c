...
{
    WORD mask[BUILTIN_IBLKS], data[BUILTIN_IBLKS];
    ...
    for (i = 0; i < BUILTIN_IBLKS; i++)
        mask[i] = data[i] = -1;
    for (i = 0, num_mask = num_data = 0; i < BUILTIN_IBLKS; i++)
    {
        if (mask[i] < 0)
        {
            mask[i] = num_mask;
            addr = icon_rs_iconblk[i].ib_pmask;
            for (j = i+1; j < BUILTIN_IBLKS; j++)
    ...
    memset(copied, 0x00, BUILTIN_IBLKS);
    for (i = 0, p = maskstart; i < BUILTIN_IBLKS; i++)
    {
        n = mask[i];
    ...
    memset(copied, 0x00, BUILTIN_IBLKS);
    for (i = 0, p = datastart; i < BUILTIN_IBLKS; i++)
    {
        n = data[i];
